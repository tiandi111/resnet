...
class ResNet(object):
	...
	def _output_layer(self, inputs, training):
		...
		### YOUR CODE HERE
		outputs = tf.layers.average_pooling2d(inputs, inputs.shape[1:3], strides=(1, 1))
		outputs = tf.reshape(outputs, [-1, outputs.shape[3]])
	...
	def _stack_layer(self, inputs, filters, block_fn, strides, training):
		...
		for i in range(self.resnet_size):
			if i == 0:
				inputs = block_fn(inputs, filters_out, training, projection_shortcut, strides)
			else:
				inputs = block_fn(inputs, filters_out, training, None, 1)
		outputs = inputs
		### END CODE HERE
